...
Surveys_to_include <- c("Triennial", "WCGBTS", "BCs", "BCt","AK_DOM_LL", "GOA", "EBS")[c(1,2,4,5,6)] #This will only work for years after 2003
...
Zone = 5
Options =  c("Calculate_Range"=FALSE, "Calculate_effective_area"=TRUE, "SD_site_logdensity"=FALSE)
...
if( "AK_DOM_LL" %in% Surveys_to_include ){
  ...
  Data5 <- read.csv(paste0(DataFile,"AK/LLData/merged_AK_DOM_LL.csv") )%>% 
    mutate(AreaSwept_km2 = 0.01, Vessel = "AK_DOM_VESSEL") %>%
...
# Make catchability matrix (Q_i) -- this should be where the BC one gets separated. 
...
plot_residuals( Lat_i=Data_Geostat[,'Lat'], Lon_i=Data_Geostat[,'Lon'], 
                extrapolation_list = Extrapolation_List,
                TmbData=TmbData, Report=Report, Q=Q, working_dir=DateFile, spatial_list=Spatial_List )
# Plot density
