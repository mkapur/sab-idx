...
Surveys_to_include <- c("Triennial", "WCGBTS", "BCs", "BCt","AK_DOM_LL", "GOA", "EBS")[c(1,2,3,5,6)] #This will only work for years after 2003


# Date
Date <- Sys.Date()
BaseQ <- c("GOA_late","AK_DOM_late", "WCGBTS")[2]
...
if( "BCs" %in% Surveys_to_include ){
  ...
  BCs <- read.csv(paste0(DataFile,"/BC/BC_sable_survey_data.Aug262019.csv"))  %>% 
    ...
    mutate(AreaSwept_km2=CPUE_TRAPS*DURATION_MINUTES,
