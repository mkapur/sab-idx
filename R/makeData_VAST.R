...
wc.combo <- Out %>%
  ...
  select(Survey, Catch_KG = total_catch_wt_kg, Year, Vessel, AreaSwept,
...
ak.goa <- read.csv( here('data',"race_cpue_by_haul-011322.csv"), header=TRUE,skip = 7) %>% 
  filter( Gear.Depth <= 700 & !(Year %in% c(1984,1987)) ) %>%
  mutate(Vessel = as.factor(Vessel.Number),
         Catch_KG = ifelse(is.na(Weight..kg.),0,Weight..kg.),
