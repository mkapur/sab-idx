...
allsubareas <- rbind(clips[[1]],clips[[2]])
...
#   spdf <- sf::as_Spatial(clips[[i]])
#   sps <- spTransform(spdf, 
...
#   ## Convert back to lon/lat coordinates as that is what VAST uses
#   region_grid_LL <- as.data.frame(spTransform(region_grid_sp, crs_LL))
#   region_grid_LL$Region_Number  <- i
#   region_grid_LL$Region_Name <- clips[[i]]$Region_Name  
#   region_grid_LL$Country_Name  <- clips[[i]]$Country_Name   
#   region_df <- with(region_grid_LL,
#                     data.frame(Lon=coords.x1,
#                                Lat=coords.x2, 
#                                # Id =NA,
...
#   ## This is the final file needed.
#   str(region)
#   saveRDS(region, here('data',paste0('regionExtrap',i,'.rds')))
# }

# ## now load and combine
# make_extrapolation_info(regionExtrap1,create_strata_per_region = TRUE)
# combi <- combine_extrapolation_info(regionExtrap1,regionExtrap2,create_strata_per_region = TRUE)
