...
# Date
Date <- Sys.Date()
BaseQ <- c("GOA_late","AK_DOM_late", "AK_DOM_LL","WCGBTS")[4]
...
if( "Triennial" %in% Surveys_to_include ){
  ...
  Data2 <- ThorsonUtilities::rename_columns( Data2[,c('Year','Latitude_dd','Longitude_dd','AreaSwept_km2','total_catch_wt_kg',"Vessel")], 
                                             newname=c('Year','Lat','Lon','AreaSwept_km2','Catch_KG',"Vessel"))
  ## Create timeblocking
  Triennial_final <- subset(Data2, Year == 2004) %>% mutate(Survey = 'Triennial_final', Region = "CC")
  
  Triennial_late <- subset(Data2, Year > 1995 & Year < 2004) %>% mutate(Survey = 'Triennial_late', Region = "CC")
  Triennial_early <- subset(Data2, Year <= 1995) %>% mutate(Survey = 'Triennial_early', Region = "CC")
  Data_CPUE = rbind( Data_CPUE,Triennial_final, Triennial_late,  Triennial_early)
  rm(Data2)
}
...
plot_residuals( Lat_i=Data_Geostat[,'Lat'], Lon_i=Data_Geostat[,'Lon'], 
                extrapolation_list = Extrapolation_List,
                TmbData=TmbData, Report=Report, Q=Q, working_dir=DateFile, 
                spatial_list=Spatial_List )
